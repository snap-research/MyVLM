...
from vlms.minigpt4.models.base_model import BaseModel
from transformers import StoppingCriteriaList

from vlms.minigpt4.conversation.conversation import StoppingCriteriaSub

class MiniGPTBaseOutput(NamedTuple):
    loss: torch.Tensor
    attentions: torch.Tensor
    concept_embed_idx: int
...
class MiniGPTBase(BaseModel):
    ...
    def preparing_embedding(self, samples, concept_signals: torch.Tensor = None):
        ### prepare input tokens
        if 'image' in samples:
            img_embeds, img_atts = self.encode_img(samples["image"], concept_signals=concept_signals)
        else:
    ...
    def forward(self, samples, reduction='mean', concept_signals: torch.Tensor = None, output_attentions: bool = True):
        # prepare the embedding to condition and the embedding to regress
        cond_embeds, cond_atts, regress_embeds, regress_atts, part_targets = \
            self.preparing_embedding(samples, concept_signals=concept_signals)
        ...
        # Note: we assume that the concept_embed_idx is fixed since we use one instruction during training.
        return MiniGPTBaseOutput(loss=loss, attentions=attentions, concept_embed_idx=3 + 256 + 1)
    ...
    def generate(
        ...
        img_embeds, atts_img = self.encode_img(images.to(self.device), concept_signals=concept_signals)
