...
class Blip2MLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, concept_signals: torch.Tensor = None) -> torch.Tensor:
        # Differentiate between the GRACE layers and regular linear layers
        hidden_states = self.fc1(hidden_states)
        hidden_states = self.activation_fn(hidden_states)
        hidden_states = self.fc2(hidden_states, concept_signals) if type(self.fc2) == MyVLMLayer else self.fc2(
            hidden_states)
...
class Blip2EncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mlp(hidden_states, concept_signals=concept_signals)

        if hidden_states.shape[1] != residual.shape[1]:
            # Add a zero for the new index that we added
            n_residuals = hidden_states.shape[1] - residual.shape[1]
            residual = torch.cat([residual,
                                  residual.new_zeros((residual.shape[0], n_residuals, residual.shape[-1]))], dim=1)
...
class Blip2Encoder(nn.Module):
    ...
    def forward(
            ...
            concept_signals: Optional[torch.Tensor] = None,
    ) -> Union[Tuple, Tuple[BaseModelOutput, torch.Tensor]]:
        r"""
        Args:
...
class Blip2VisionModel(Blip2PreTrainedModel):
    ...
    def forward(
            ...
            output_hidden_states: Optional[bool] = None,
            return_dict: Optional[bool] = None,
            concept_signals: Optional[torch.FloatTensor] = None,
...
class Blip2QFormerMultiHeadAttention(nn.Module):
    ...
    def forward(
            ...
            normalize_concept_embedding: Optional[bool] = True,
    ):
        ...
        if is_cross_attention:
            keys_ = self.key(encoder_hidden_states)
            values_ = self.value(encoder_hidden_states)

            if encoder_hidden_states.shape[1] > 257 and normalize_concept_embedding:
                # Normalize the norm of the added tokens to match the cls token
                keys = keys_.clone()
                values = values_.clone()
                keys_norm_factor = keys_[:, 257:, :].norm(dim=-1, keepdim=True)
                values_norm_factor = values_[:, 257:, :].norm(dim=-1, keepdim=True)
                # Normalize by the average norm of the image tokens
                new_key_norm_value = keys_[:, :257, :].norm(dim=-1, keepdim=True).mean(dim=1).unsqueeze(-1)
                new_value_norm_value = values_[:, :257, :].norm(dim=-1, keepdim=True).mean(dim=1).unsqueeze(-1)
                keys[:, 257:, :] = keys_[:, 257:, :].clone() / keys_norm_factor * new_key_norm_value
                values[:, 257:, :] = values_[:, 257:, :].clone() / values_norm_factor * new_value_norm_value
            else:
                keys = keys_
                values = values_
...
class Blip2QFormerAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs, attention_probs = self.attention(
            hidden_states,
            attention_mask,
            head_mask,
            ...
            past_key_value,
            output_attentions,
            normalize_concept_embedding=normalize_concept_embedding
        )
    ...
    def forward(
        ...
        if query_length > 0:
            ...
            if self.has_cross_attention:
                ...
                cross_attention_outputs, attention_probs = self.crossattention(
    ...
    def forward(
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                layer_outputs, layer_attention_probs = torch.utils.checkpoint.checkpoint(
                    ...
                    encoder_hidden_states,
                    encoder_attention_mask,
                    normalize_concept_embedding
                )
            else:
                layer_outputs, layer_attention_probs = layer_module(
                    hidden_states,
                    attention_mask,
                    layer_head_mask,
                    encoder_hidden_states,
...
class Blip2QFormerModel(Blip2PreTrainedModel):
    ...
    def forward(
            ...
            return_dict=None,
            normalize_concept_embedding=True
    ):
        r"""
        ...
        encoder_outputs, attention_probs = self.encoder(
            embedding_output,
...
class Blip2ForConditionalGeneration(Blip2PreTrainedModel):
    ...
    def __init__(self, config: Blip2Config):
        ...
        # For trying to apply grace on the query tokens
        self.grace_query_adaptor = None
    ...
    def forward(
        ...
        query_outputs, attention_probs = self.qformer(
            query_embeds=query_tokens,
            encoder_hidden_states=image_embeds,
            encoder_attention_mask=image_attention_mask,
            output_attentions=output_attentions,
    ...
    def generate(
        ...
        batch_size = pixel_values.shape[0]
        image_embeds = self.vision_model(pixel_values, concept_signals=concept_signals, return_dict=True)
        image_embeds = image_embeds.last_hidden_state
        image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.long, device=image_embeds.device)

        query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1)

        query_outputs, _ = self.qformer(
            query_embeds=query_tokens,
            encoder_hidden_states=image_embeds,
            encoder_attention_mask=image_attention_mask,
            return_dict=True,
